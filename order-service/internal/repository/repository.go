...
func (r *Repository[T]) FindAll(db *gorm.DB, entities *[]T, pagination *utilsShared.PaginationRequest) (int64, error) {
	var total int64

	query := db.Model(new(T))

	// cek apakah entity implement Searchable
	if s, ok := any(new(T)).(utilsShared.Searchable); ok && pagination.Search != "" {
